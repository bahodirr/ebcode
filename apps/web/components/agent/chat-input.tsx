...
const TIERS = {
  intern: { model: "big-pickle", provider: "opencode", label: "Big Pickle — (Free)" },
  en: { model: "gpt-5.1-codex-max", provider: "openai", label: "OpenAI GPT-5 — Pro" },
  coder: { model: "grok-4-1-fast", provider: "xai", label: "xAI Grok-4 — Pro" },
} as const;

const MAX_FILES = 5;
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB

export default function ChatInput({ onSubmit, disabled, placeholder = "Ask anything...", className, mode = "plan", onToggleMode, isWorking, onStop, isStopping, header }: Props) {
  const [value, setValue] = useState("");
  const [tier, setTier] = useState<keyof typeof TIERS>("intern");
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={tier} onValueChange={v => setTier(v as keyof typeof TIERS)}>
              <SelectTrigger size="sm" className="h-8 rounded-full border-0 bg-transparent px-3 text-xs w-auto hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 transition-colors shadow-none focus:ring-0">
                <span className="flex items-center gap-1">{TIERS[tier].label}</span>
              </SelectTrigger>
              <SelectContent>
                {Object.entries(TIERS).map(([k, v]) => (
                  <SelectItem key={k} value={k}>
                    <span className="text-sm text-zinc-800 dark:text-zinc-100">{v.label}</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
