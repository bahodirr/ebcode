...
import { ChevronDown, ChevronUp, Undo2, CheckCircle2, Eye, FileText, FilePenLine, Trash2, Terminal, Search, Globe, ListTodo, Play, Loader2 } from "lucide-react";
...
// Main
export function AgentThread({ messages, partsMap, onRevert, revertMessageId, reverting, revertingMessageId, onViewChanges, compact, onToggleCompact }: {
  sessionId: string;
  messages: Message[];
  ...
  if (compact) {
    ...
    if (lastMsg) {
      if (lastMsg.role === "user") {
        ...
        const parts = partsMap[lastMsg.id] || [];
        // Find active part
        const activePart = parts.slice().reverse().find(p => 
          (p.type === "tool" && ["running", "pending"].includes((p as ToolPart).state.status)) ||
        ...
        if (activePart) {
          if (activePart.type === "reasoning") {
             ...
             const toolPart = activePart as ToolPart;
             const cfg = TOOLS[toolPart.tool] || { icon: FileText, done: toolPart.tool, doing: "Working..." };
             const target = getTarget(toolPart);
             status = `${cfg.doing} ${target ? `for ${target}` : ""}`;
          }
        } else {
          // Check for last text or completed tool
          const lastPart = parts[parts.length - 1];
          const isStreaming = !lastMsg.time?.completed;
          if (lastPart?.type === "text") {
            status = isStreaming ? "Agent is sending message..." : "Message sent";
          } else if (lastPart?.type === "tool") {
    ...
    return (
      <div className="flex items-center gap-2 text-xs cursor-pointer pb-1" onClick={onToggleCompact}>
        {status}
      </div>
    );
  }
        ...
        return (
          <div key={userMsg.id} className="space-y-3">
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              <div className="relative max-w-[80%] rounded-lg bg-muted/40 border px-2.5 py-1.5">
                <div className="whitespace-pre-wrap text-sm pr-5">{text}</div>
                {onRevert && (
                  <Tooltip>
                    <TooltipTrigger asChild>
